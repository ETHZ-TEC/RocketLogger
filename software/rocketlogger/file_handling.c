        ...
        if (conf->channels[i] == CHANNEL_ENABLED) {
            ...
            if (is_current(i)) {
                ...
                if (is_low_current(i)) {
                    ...
                    if (i == I1L_INDEX) {
                        file_header->channel[j].valid_data_channel =
                            i1l_valid_channel;
                    } else {
                        file_header->channel[j].valid_data_channel =
                            i2l_valid_channel;
...
void store_header_csv(FILE* data, struct rl_file_header* file_header) {
    ...
    // channels
    for (i = 0; i < (file_header->lead_in.channel_count +
 ...
 */
void merge_currents(uint8_t* valid, int64_t* dest, int64_t* src,
                    struct rl_conf* conf) {
 ...
 */
void handle_data_buffer(FILE* data, void* buffer_addr, uint32_t sample_size,
                        uint32_t samples_buffer, struct rl_conf* conf,
                        int sem_id, struct web_shm* web_data_ptr) {
    ...
    uint32_t avg_length[WEB_RING_BUFFER_COUNT] = {
        samples_buffer / BUFFER1_SIZE, samples_buffer / BUFFER10_SIZE,
                ...
                if (conf->sample_rate == 10) {
                    ...
                    if (bin_channel_pos > 0) {
                        if (conf->file_format == BIN) {
                            ...
                            if (conf->channels[I1L_INDEX] == CHANNEL_ENABLED) {
                                temp_bin = temp_bin |
                                           (avg_valid[BUF10_INDEX][0] << j++);
                            }
                            if (conf->channels[I2L_INDEX] == CHANNEL_ENABLED) {
                                temp_bin = temp_bin |
                                           (avg_valid[BUF10_INDEX][1] << j++);
                            }
                ...
                if (conf->sample_rate == 1) {
                    ...
                    if (bin_channel_pos > 0) {
                        if (conf->file_format == BIN) {
                            ...
                            if (conf->channels[I1L_INDEX] == CHANNEL_ENABLED) {
                                temp_bin = temp_bin |
                                           (avg_valid[BUF100_INDEX][0] << j++);
                            }
                            if (conf->channels[I2L_INDEX] == CHANNEL_ENABLED) {
                                temp_bin = temp_bin |
                                           (avg_valid[BUF100_INDEX][1] << j++);
                            }
    ...
    if (conf->enable_web_server == 1) {
        ...
        if (wait_sem(sem_id, DATA_SEM, SEM_WRITE_TIME_OUT) == TIME_OUT) {
            ...
            // write time
            web_data_ptr->time =
                time_real.sec * 1000 + time_real.nsec / 1000000;

            // write data to ring buffer
            buffer_add(&web_data_ptr->buffer[BUF1_INDEX],
                       &web_data[BUF1_INDEX][0][0]);
            buffer_add(&web_data_ptr->buffer[BUF10_INDEX],
                       &web_data[BUF10_INDEX][0][0]);
            buffer_add(&web_data_ptr->buffer[BUF100_INDEX],
