...
cp -f ROCKETLOGGER.dts bb.org-overlays/src/arm
(cd bb.org-overlays && make src/arm/ROCKETLOGGER.dtbo)

# deploy device tree binary
sudo cp -f bb.org-overlays/src/arm/ROCKETLOGGER.dtbo /lib/firmware
...
sudo cp -f rocketlogger.conf /etc/modprobe.d/
